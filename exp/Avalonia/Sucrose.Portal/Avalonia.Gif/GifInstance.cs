...
namespace Avalonia.Gif
{
    public class GifInstance : IDisposable
    {
        ...
        {
            if (!Uri.TryCreate(str, UriKind.RelativeOrAbsolute, out Uri? res))
            {
        ...
        {
            string uriString = uri.OriginalString.Trim();
        ...
        public WriteableBitmap? ProcessFrameTime(TimeSpan stopwatchElapsed)
        {
            ...
            long elapsedTicks = stopwatchElapsed.Ticks;
            TimeSpan timeModulus = TimeSpan.FromTicks(elapsedTicks % _totalTime.Ticks);
            TimeSpan targetFrame = _frameTimes.FirstOrDefault(x => timeModulus < x);
            int currentFrame = _frameTimes.IndexOf(targetFrame);
            if (currentFrame == -1)
            {
                currentFrame = 0;
            }
